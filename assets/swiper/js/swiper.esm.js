...
class SwiperClass {
	constructor(params = {}) {
		...
		if (self.params && self.params.on) {
			Object.keys( self.params.on ).forEach(
				(eventName) => {
	...
	useModulesParams(instanceParams) {
		...
		Object.keys( instance.modules ).forEach(
			(moduleName) => {
	...
	useModules(modulesParams = {}) {
		...
		Object.keys( instance.modules ).forEach(
            ...
            // Extend instance methods and props
				if (module.instance) {
					Object.keys( module.instance ).forEach(
						(modulePropName) => {
	...
	static installModule(module, ...params) {
		...
		// Prototype
		if (module.proto) {
			Object.keys( module.proto ).forEach(
				(key) => {
		...
		// Class
		if (module.static) {
			Object.keys( module.static ).forEach(
				(key) => {
		...
		swiper.slideToClickedSlide();
					...
					class Swiper extends SwiperClass {
						constructor(...args) {
							...
							Object.keys( prototypes ).forEach(
								(prototypeGroup) => {
                                Object.keys( prototypes[prototypeGroup] ).forEach(
										(protoMethod) => {
							...
							Object.keys( swiper.modules ).forEach(
								(moduleName) => {
					...
					const Observer              = {
						...
						attach( target, options = {} ) {
							...
							const observer     = new ObserverFunc(
								(mutations) => {
						...
						destroy() {
							const swiper = this;
							swiper.observer.observers.forEach(
								(observer) => {
                                observer.disconnect();
								}
							);
					...
					const Virtual        = {
						...
						if (swiper.params.virtual.renderExternal) {
											swiper.params.virtual.renderExternal.call(
												swiper,
												{
													offset,
													from,
						...
						appendIndexes.forEach(
							(index) => {
                            swiper.$wrapperEl.append( renderSlide( slides[index], index ) );
							}
						);
						prependIndexes.sort( (a, b) => b - a ).forEach(
							(index) => {
					...
					var zoom             = {
						...
						create() {
							...
							('onGestureStart onGestureChange onGestureEnd onTouchStart onTouchMove onTouchEnd onTransitionEnd toggle enable disable in out').split( ' ' ).forEach(
								(methodName) => {
					...
					const Lazy                              = {
						...
						load() {
							...
							if (swiper.params.watchSlidesVisibility) {
												$wrapperEl.children( `.${swiperParams.slideVisibleClass}` ).each(
													(elIndex, slideEl) => {
					...
					const a11y           = {
						...
						updatePagination() {
							...
							if (swiper.pagination && swiper.params.pagination.clickable && swiper.pagination.bullets && swiper.pagination.bullets.length) {
												swiper.pagination.bullets.each(
													(bulletIndex, bulletEl) => {
